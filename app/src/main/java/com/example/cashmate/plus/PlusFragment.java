...
public class PlusFragment extends Fragment {
    ...
    private String selectedGroupName = null; // Đây là tên nhóm (VD: "Ăn uống")
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        getParentFragmentManager().setFragmentResultListener("select_group", this, (k, b) -> {
            selectedCategoryId = b.getLong("idCategory");
            selectedGroupName = b.getString("name"); // Lấy tên nhóm
        ...
        getParentFragmentManager().setFragmentResultListener("note_result", this, (k, b) -> {
    ...
    // ... (Các hàm loadTransactionForEdit, DatePicker, UI giữ nguyên) ...
    private void loadTransactionForEdit(long id) {
        TransactionHandle handle = new TransactionHandle(requireContext());
        Cursor c = handle.getById(id);
        if (c != null && c.moveToFirst()) {
            etAmount.setText(String.valueOf(c.getDouble(c.getColumnIndexOrThrow("amount"))));
            ...
            updateGroupUI();
            selectedDate = LocalDate.parse(c.getString(c.getColumnIndexOrThrow("date")), DateTimeFormatter.ofPattern("dd/MM/yyyy"));
            updateDateUI();
        }
        if (c != null) c.close();
    }
    ...
    private void openSpinnerDatePicker() {
        DatePickerDialog dialog = new DatePickerDialog(requireContext(), android.R.style.Theme_Holo_Light_Dialog_NoActionBar, (picker, y, m, d) -> {
            selectedDate = LocalDate.of(y, m + 1, d);
            updateDateUI();
        }, selectedDate.getYear(), selectedDate.getMonthValue() - 1, selectedDate.getDayOfMonth());
        dialog.getDatePicker().setCalendarViewShown(false);
        dialog.getDatePicker().setSpinnersShown(true);
        Calendar min = Calendar.getInstance(); min.set(2000, Calendar.JANUARY, 1);
        Calendar max = Calendar.getInstance(); max.set(2050, Calendar.DECEMBER, 31);
    ...
    private void updateDateUI() {
        tvDate.setText(getThu(selectedDate.getDayOfWeek()) + ", " + selectedDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    }
    ...
    private void updateGroupUI() {
        if (selectedGroupName != null) {
            ...
            int res = getResources().getIdentifier(selectedGroupIcon, "drawable", requireContext().getPackageName());
